...
MMD_l <- function(x_obs, x_sim, sigma = 1, permute = FALSE){
  ...
  if(permute){
    x_obs = sample(x_obs, m2*2, replace = FALSE)
    x_sim = sample(x_sim, m2*2, replace = FALSE)

    x <- split(x_sim, 1:2)
    y <- split(x_obs, 1:2)

    # x <- list(.subset(x_sim, 1:m2), .subset(x_sim, I(m2+1)))
    # y <- split(x_obs, 1:2)
...
#' @export
MMD_l_old <- function(x_obs, x_sim, sigma = 1, permute = FALSE){
